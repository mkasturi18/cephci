...
def run(ceph_cluster, **kw):
    ...
    # When using IBM key: remove root-owned io_info_<config_file_name> so test can (re)write it,
    # and export IBM_CLOUD_API_KEY from the key file for the test script.
    if ibm_key_file:
        exec_from.exec_command(
            cmd=f"sudo rm -f ~/io_info_{config_file_name}",
            check_ec=False,
        )
        # env expects KEY=value pairs, not "export KEY=value" (env would try to run "export")
        export_ibm_key = f"IBM_CLOUD_API_KEY=$(cat {ibm_key_file})"
        cmd_env = f"{export_ibm_key} {cmd_env}" if cmd_env else export_ibm_key
    # Run test script with sudo (scripts may need /etc/ceph write, etc.)
    if cmd_env:
