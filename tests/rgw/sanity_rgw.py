...
def run(ceph_cluster, **kw):
    ...
    # install kafka packages and start zookeeper and kafka services
    if install_start_kafka:
        log.info("install and start kafka broker for bucket notification tests")
        rgw_node.exec_command(
            cmd="wget http://magna002.ceph.redhat.com/cephci-jenkins/kafka_2.13-2.8.0.tgz && tar -zxvf kafka_2.13-2.8.0.tgz && mv kafka_2.13-2.8.0 kafka", long_running=True
        )
