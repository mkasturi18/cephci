...
def setup_s3_tests(client_node, rgw_node, config, build):
    ...
    if build.startswith('4'):
        ...
        client_node.exec_command(cmd="sudo yum install -y python2-virtualenv python2-devel libevent-devel libffi-devel "
                                     "libxml2-devel libxslt-devel zlib-devel", check_ec=False)
...
def execute_s3_tests(client_node, build):
    ...
    try:
        if build.startswith('4'):
            cmd = "cd s3-tests; S3TEST_CONF=config.yaml ~/virtualenv/bin/nosetests -v -a"
        else:
